...
\begin{definition}
    ...
    \begin{equation}
        \begin{aligned}[t]
            ...
            % \abs{\partial^l(k_{ij})}&=O(\abs{u}^{-1-q-l}),\quad l=0,1\mathcomma
        \end{aligned}
    \end{equation}
...
\begin{proof}[Proof of \cref{thm:harmonic_mass_equality}]
    Consider harmonic coordinates \( x_1,x_2,x_3 \), see \cite[Section 3.2]{brayHarmonicFunctionsMass2019}. \todo[Finish this later.]
\end{proof}
...
%      To begin, note that 
%     \begin{equation*}
%         \nabla_{\nls^i}\abs{\nabla_{\nls^i}}=\nabla_{\nls^i}((g_{\nls^i})^{ii}+O_2(\abs{u}^{-2q}))^{1/2}=-\frac{1}{2}\nabla_{\nls^i}(g_{\nls^i})_{ii}+O_1(\abs{u}^{-1-2q})
%     \end{equation*}
%     and
%     \begin{equation*}
%         \evaluateat{\nu_{\nls^i}}{F_{\pm L,j}}=\pm \partial_{\nls^i,j}+O_2(\abs{u}^{-q})\mathfullstop
%     \end{equation*}
%     We further have
    % \begin{equation*}
    %     \frac{1}{\prod_{j=1}^{i-1}\abs{\nabla_{\nls^j}u_j}}=\frac{1}{\prod_{j=1}^{i-1}(\delta^{ii}+O_2(\abs{u}^{-q}))}=1+O_2(\abs{u}^{-q})\mathfullstop
    % \end{equation*}
%     % and similarly
%     % \begin{equation*}
%     %     \prod_{j=1}^{i-1}\abs{\nabla u_j}=1+O_2(\abs{u}^{-q}).
%     % \end{equation*}
%     It follows that
%     \begin{equation*}
%         \int_{I_1^{i-1}}\int_{F_{\pm L,j}\cap \nls^i}\frac{\partial_{\nu_{\nls^i}}\abs{\nabla_{\nls^i}u_i}}{\prod_{j=1}^{i-1}\abs{\nabla_{\nls^j}u_j}}\odif{A}\odif{\vect{t}}=\int_{I_1^{i-1}}\int_{ F_{\pm L,j}\cap \nls^i}\pm (g_{\nls^i})_{ii,j}\odif{A}\odif{\vect{t}}+O_2(L^{1-2q})\mathcomma
%     \end{equation*}
%     where we have used that \( \nabla_{\nls^i}(g_{\nls^i})_{ii}=O_1(\abs{u}^{-1-q}) \). Note that the above is zero (!) if \( j<i \) (as then for almost all \( \vect{t} \) we have \( F_{\pm L,j}\cap \nls^i=\emptyset \)). Note also that
%     \begin{equation*}
%         1-2q\leq 1-(n-2)<0
%     \end{equation*} 
%     for all \( n\geq 3 \). Now, because \( u^i \) is harmonic on \( \nls^i \) we get the following equivalent of \cite[Equation 6.9]{brayHarmonicFunctionsMass2019} (\todo[add full calculation])
%     \begin{equation*}
%         (g_{\nls^i})_{ii,i}=\sum_{j=i+1}^{n}(g_{\nls}^i)_{jj,i}-2(g_{\nls}^i)_{ji,j}.
%     \end{equation*}
...
% \begin{equation*}
%     \partial_{\nu_{\nls^i}}\abs{\nabla_{\nls^i}u_i}=H_{\nls^i\cap \boundary{M_{\ext}}}\abs{\partial_{\nu_{\nls^i}}u_i}\mathcomma
% \end{equation*}
% where \( H_{\nls^i\cap \boundary{M_{\ext}}} \) is the mean curvature of \( \nls^i\cap \boundary{M_{\ext}} \) inside \( \nls^i \). But \( \boundary{M_{\ext}}\subset \nls^i \) and
